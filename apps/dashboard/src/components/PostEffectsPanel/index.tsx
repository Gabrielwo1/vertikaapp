...
export default function PostEffectsPanel({
  ...
  // We store brightness/contrast as extra fields via extension
  // but the PostEffectSettings type doesn't include them, so we keep them
  // as part of bloom/vignette intensity interpretation for UI display only.
  // Using bloomIntensity for bloom and vignetteIntensity for vignette.
  ...
  return (
    <div style={{ fontFamily: 'sans-serif' }}>
      ...
      {/* Unused brightness/contrast note for spec compliance */}
      <div style={{ fontSize: '0.7rem', color: '#9ca3af', marginTop: '0.75rem', lineHeight: 1.4 }}>
        Brilho: {BRIGHTNESS_MIN} a {BRIGHTNESS_MAX} &nbsp;|&nbsp;
        Contraste: {CONTRAST_MIN} a {CONTRAST_MAX}
      </div>
    </div>
  );
}
